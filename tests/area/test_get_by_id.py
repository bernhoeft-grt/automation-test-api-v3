...
@allure.feature("Area")
@allure.story("GET /api/v1/area/{id}")
class TestGetAreaById:
    """Test GET Area by ID."""
    
    @allure.title("Get Area by ID")
    @pytest.mark.api
    def test_get_by_id(self, api_client):
        ...
        if all_response.status_code == 200 and all_response.json():
            ...
            if test_id:
                with allure.step(f"Make GET request to /api/v1/area/{test_id}"):
                    response = page.get_by_id(test_id)
                    attach_request("GET", f"/area/{test_id}")
